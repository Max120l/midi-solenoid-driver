...
| **Service** | reset the boards; the 12 V supply and the pump on and off by hand; Shut down, which turns the pump and the supply off and then powers the Pi off, so the panel switch can be turned off safely once the screen is dark; the keys tester, by section or by board, with hold mode, the snare roll, a repeat mode that restrikes one pipe at the interval slider's rate (for finding how fast each rank can repeat), and the stops: each register decoded from its name onto the section it colours ("Trumpet Melody" is the Trumpet stop above the Melody keys), a tap throwing it the other way and All stops off pulsing every reset coil in turn. A register sits wherever its last pulse put it, and every pulse is accounted for: the desk resets them all the first time the keys are used after a start, each tune resets them at its start and end, and the player clears them when a tune is cut short, so the buttons show where the registers are. Usable when the player is idle |
...
| `GET /api/keys/layout`, `POST /api/keys/pulse` `/hold` `/roll` `/off` | `{solenoid, ms?}`, `{solenoid,on}`, `{solenoids?,interval_ms?,on}` | the keys tester; the layout lists the stops with their section and last commanded state |
| `POST /api/keys/register`, `POST /api/keys/stops_off` | `{name, on}`, | throw one stop by name (its set or reset coil, `register_pulse_ms`); pulse every reset coil a stagger apart. Every keys reply carries `stops`, the state per register, `null` only before the first use since the desk started |
