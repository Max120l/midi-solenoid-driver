...
| `POST /api/keys/register`, `POST /api/keys/stops_off` | `{name, on}`, | throw one stop by name (its set or reset coil, `register_pulse_ms`); pulse every reset coil a stagger apart. Every keys reply carries `stops`, the last commanded state per register, `null` when unknown |
